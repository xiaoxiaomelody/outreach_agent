...
const MyListPage = () => {
  ...
  const loadContactsFromFirestore = async (userId) => {
    try {
      ...
      const hasData =
        firestoreContacts.shortlist?.length > 0 ||
      ...
      if (hasData) {
        ...
        if (savedContacts) {
          ...
          const hasLocalData =
            localContacts.shortlist?.length > 0 ||
            localContacts.sent?.length > 0 ||
            localContacts.trash?.length > 0;
          if (hasLocalData) {
            console.log(
              "📋 Found data in localStorage, using it and migrating to Firestore"
            );
            ...
            console.log(
              "📋 No data in localStorage either, using empty arrays"
            );
            setContacts(firestoreContacts);
          }
        } else {
