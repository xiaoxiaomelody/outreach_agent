            ...
            if (template) {
                ...
                try {
                    const result = await gmailApi.draftEmail({
                        ...
                        recipientSummary: contact.ai_summary || `${fullName} works as ${contact.position} at ${contact.company || contact.organization}`,
                    ...
                    if (result.success && result.data) {
                        ...
                        setSubject(result.data.subject || `Outreach: ${fullName} at ${contact.company || contact.organization || "Company"}`);
                    } else {
                        ...
                        setSubject(`Outreach: ${fullName} at ${contact.company || contact.organization || "Company"}`);
                    }
                } catch (error) {
                    // Fallback to simple replacement
                    const personalizedBody = template.content
    ...
    const handleSend = async () => {
        ...
        try {
            ...
            if (!gmailStatus.success || !gmailStatus.data.connected) {
                alert("⚠️ Please connect your Gmail account first using the 'Connect Gmail' button in Search page.");
                setSending(false);
                return;
            }
            ...
            if (result.success) {
                ...
                const contacts = JSON.parse(localStorage.getItem("myContacts") || '{"shortlist":[],"sent":[],"trash":[]}');
                const email = contact.value || contact.email;
                contacts.shortlist = contacts.shortlist.filter(c => (c.value || c.email) !== email);
                const existsInSent = contacts.sent.some(c => (c.value || c.email) === email);
                if (!existsInSent) {
                ...
                <div className="preview-content">
                    <div className="preview-field">
                        ...
                        {isEditing ? (
                            ...
                            <div className="preview-subject">{subject || "[Subject]: Please hire me!"}</div>
                        )}
                    </div>
                    <div className="preview-field">
                        <label>Body:</label>
