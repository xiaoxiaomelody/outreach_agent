    ...
    const checkGmailStatus = async () => {
        ...
        try {
            ...
            if (result.success) {
                ...
                if (onStatusChange) {
                    console.log('📧 [STATUS CHECK] Calling onStatusChange with:', result.data.connected);
                    onStatusChange(result.data.connected);
                } else {
                    console.warn('📧 [STATUS CHECK] onStatusChange callback not provided!');
                }
            } else {
                    ...
                    try {
                        ...
                        if (statusStr) {
                            ...
                            if (status.connected && (Date.now() - status.timestamp) < 10000) {
                                console.log('✅ [CONNECT] Found connection status in localStorage:', status.email);
